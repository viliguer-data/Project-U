...
if (name1Select && previewDiv) {
  name1Select.addEventListener('change', () => {
    ...
    if (itemId && typeof COMPENDIUM !== 'undefined' && COMPENDIUM[itemId]) {
      ...
      if (imgPath) {
        const tooltipContent = getMultilingualTooltip(item, window.lang);
...
if (name2Select && previewDiv2) {
  ...
  function getMultilingualTooltip(item, currentLang) {
  if (!item) return '';
  
  const languages = [
    { code: 'es', name: 'Español', emoji: '🇪🇸' },
    { code: 'en', name: 'English', emoji: 'EN' },
  ...
  // Construir lista de nombres por idioma
  let namesList = '';
  languages.forEach(lang => {
    const translatedName = getItemName(item, lang.code);
    // Resaltar el idioma actual
    const isCurrent = (lang.code === currentLang);
    const highlight = isCurrent ? ' style="color:var(--gold);font-weight:bold;"' : '';
    namesList += `<div${highlight}>${lang.emoji} ${translatedName}</div>`;
  });
  
  // Stats y requisitos (en el idioma actual)
  ...
  return `
    <div class="multilang-tooltip">
      <div class="tooltip-names">${namesList}</div>
...
function wmPopulateNames(hand) {
  ...
  const selectedSlot = typeSel.value;
    nameSel.innerHTML = `<option value="" data-i18n="weapon.name.placeholder">— ${T_raw[currentLang]?.['weapon.name.placeholder'] || 'Seleccionar arma'} —</option>`;
...
shieldSel.innerHTML = `<option value="" data-i18n="shield.none">— ${T_raw[currentLang]?.['shield.none'] || 'Ninguno'} —</option>`;
...
slotDiv.innerHTML = `
  <div class="gear-slot-label">${T_raw[currentLang]?.[`item.slot.${slot}`] || slot.toUpperCase()}</div>
  <select id="gs-${slot}" data-slot="${slot}" class="tracker-select" style="width:100%">
    <option value="" data-i18n="ui.placeholder.select">— ${T_raw[currentLang]?.['ui.placeholder.select'] || 'Seleccionar'} —</option>
  </select>
  <div class="item-preview" style="margin-top: 8px; min-height: 50px; display: flex; justify-content: center;"></div>
`;
